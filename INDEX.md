# Index du Référentiel documentaire

## Avertissement

INDEX.md est un document de navigation.

Le contenu normatif officiel du Référentiel documentaire est défini par :

- MANIFEST.md

Le manifeste prévaut toujours en cas de divergence.

Aucun contenu de INDEX.md ne doit être interprété comme modifiant les documents normatifs du référentiel.

---

# Documents normatifs

Les documents normatifs constituent le Référentiel documentaire canonique.

| Ordre | Fichier | Sujet |
|---------|---------|---------|
| 01 | docs/A.md | Gouvernance méthodologique |
| 02 | docs/B.md | Sélection et application des directives |
| 03 | docs/C.md | Validation des sources |
| 04 | docs/D.md | Vérification des affirmations |
| 05 | docs/E.md | Recherche documentaire |
| 06 | docs/F.md | Sources académiques et scientifiques |
| 07 | docs/G.md | Citations |
| 08 | docs/H.md | Formats documentaires |
| 09 | docs/I.md | Analyses historiques |
| 10 | docs/J.md | Production de corpus documentaires |
| 11 | docs/K.md | Traduction documentaire |
| 12 | docs/L.md | Synthèse documentaire |
| 13 | docs/M.md | Production pédagogique |
| 14 | docs/N.md | Évaluation documentaire |
| 15 | docs/O.md | Directives complémentaires |

---

# Documents complémentaires

Les documents complémentaires complètent le référentiel mais ne le remplacent pas.

| Fichier | Type |
|---------|---------|
| annexes/Annexe A.md | Annexe |
| annexes/Annexe B.md | Annexe |
| annexes/Annexe C.md | Annexe |
| annexes/Annexe D.md | Annexe |

---

# Documents de gestion du référentiel

| Fichier | Description |
|---------|---------|
| README.md | Présentation générale du projet |
| MANIFEST.md | Définition officielle du référentiel canonique |
| CHANGELOG.md | Historique des modifications |
| VERSION.md | Version courante du référentiel |
| LICENCE | Conditions d'utilisation |
| INDEX.md | Index documentaire du dépôt |

---

# Ordre de lecture recommandé

Pour une compréhension complète du référentiel :

1. README.md
2. MANIFEST.md
3. docs/A.md
4. docs/B.md
5. docs/C.md
6. docs/D.md
7. docs/E.md
8. docs/F.md
9. docs/G.md
10. docs/H.md
11. docs/I.md
12. docs/J.md
13. docs/K.md
14. docs/L.md
15. docs/M.md
16. docs/N.md
17. docs/O.md
18. Annexes applicables

---

# Règle de priorité documentaire

En cas de conflit :

1. Le contenu de MANIFEST.md prévaut.
2. Les documents normatifs contenus dans `docs/` prévalent.
3. Les annexes complètent les documents normatifs.
4. README.md, INDEX.md, VERSION.md et CHANGELOG.md n'ont pas de valeur normative.

---

# Référentiel canonique

Le Référentiel documentaire canonique correspond à l'ensemble des documents identifiés dans :

`MANIFEST.md`

Aucun document non mentionné dans le manifeste ne fait partie du référentiel canonique.
